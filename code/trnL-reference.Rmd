...
```
Note that not all of these come down with a trnL marker query

```{r}
# try for a few of those missing
organism <- 'Brassica oleracea'
```

```{r}
ids <- lapply(organism_query, 
              entrez_search, 
              db='genome', use_history=TRUE)

seqs.return = NULL # Store results here
seqs.count = 0


links <- entrez_link(dbfrom = 'genome',
                     web_history = ids[[i]]$web_history,
                     db = 'nuccore')

# This is the single case: in practice, links will probably return a vector of links rather than just one

seqs <- entrez_fetch(db='nuccore', 
                     id = links$links$genome_nuccore[1],
                     rettype='fasta') 
...
strsplit('\n{1,}') %>% # Usually two newline chars, but sometimes more
     unlist()

# Check for primers on the fly to keep data manageable
...
for (i in seq_along(ids)){
     if (ids[[i]]$count==0){
          ...
          # Need to link from genome to nucleotide, and then fetch
          # that record
          ex <- '[^>]\\S*' 
          seqs <- 
...
```
Interesting that only one of these found.  What happened to LR031876.1 and LR031874.1, which were pulled in prior query? 
